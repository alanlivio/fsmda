...
for class in "${classes[@]}";do
    ...
    grep "fsmda_parent profile_pairing $class" < fsmda_profiling.log \
      |awk '{ SUM += $4; N += 1} END { printf "%d\n",SUM/N }'
done
...
for class in "${classes[@]}";do
    for ((i=1; i<=$n; i++));do
      ...
      ./fake_parent_helper --device_class=$class --application_id=$app_id --profile_prepare \
         >> fsmda_profiling.log &
      ./fake_child_helper --device_class=$class --application_id=$app_id --profile_prepare \
         >> fsmda_profiling.log
      ./release_fake_child_parent.sh
    done
    echo -n "$class child average="
    grep "fsmda_child profile_prepare $class" < fsmda_profiling.log \
      |awk '{ SUM += $4; N += 1} END { printf "%d\n",SUM/N}'
    echo -n "$class parent average="
...
for class in "${classes[@]}";do
    for ((i=1; i<=$n; i++));do
      ...
      ./fake_parent_helper --device_class=$class --application_id=$app_id --profile_command \
         >> fsmda_profiling.log &
      ./fake_child_helper --device_class=$class --application_id=$app_id --profile_command \
         >> fsmda_profiling.log
      ./release_fake_child_parent.sh
    done
    echo -n "$class child average="
    grep "fsmda_child profile_command $class" < fsmda_profiling.log \
      |awk '{ SUM += $4; N += 1} END { printf "%d\n",SUM/N}'
    echo -n "$class parent average="
...
for class in "${classes[@]}";do
    for ((i=1; i<=$n; i++));do
      ...
      ./fake_parent_helper --device_class=$class --application_id=$app_id --profile_variable \
         >> fsmda_profiling.log &
      ./fake_child_helper --device_class=$class --application_id=$app_id --profile_variable \
         >> fsmda_profiling.log
      ./release_fake_child_parent.sh
    done
    echo -n "$class child average="
    grep "fsmda_child profile_variable $class" < fsmda_profiling.log \
      |awk '{ SUM += $4; N += 1} END { printf "%d\n",SUM/N}'
    echo -n "$class parent average="
